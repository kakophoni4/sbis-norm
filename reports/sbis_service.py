...
def _probe_proxy_connectivity(
    proxy_url: str,
    *,
    test_url: str = "https://online.sbis.ru/",
    timeout: float = 5.0,
) -> tuple[bool, str]:
    """
    Быстрый тест: прокси должен поднять CONNECT и вернуть хоть какой-то HTTP-ответ от online.sbis.ru.
    Нам не важно 200/302/401/403 — важно, что это НЕ 429/5xx от прокси и не ProxyError/Timeout.
    """
    ...
    try:
        ...
        # 429/5xx часто именно от прокси/туннеля — считаем "плохой"
...
def _sbis_post(url: str, *, headers: dict, data: str, timeout: int = 30, inn: str | None = None, proxy_url_override: str | None = None):
    return _sbis_request("POST", url, headers=headers, data=data, timeout=timeout, inn=inn, proxy_url_override=proxy_url_override)


def _sbis_get(url: str, *, headers: dict, timeout: int = 60, inn: str | None = None, proxy_url_override: str | None = None):
    return _sbis_request("GET", url, headers=headers, data=None, timeout=timeout, inn=inn, proxy_url_override=proxy_url_override)
...
def auth_sbis_by_cert(cert_path: str, thumbprint: str, inn: str = "no_inn", *, proxy_url: str | None = None) -> str:
    ...
    resp = _sbis_request(
        ...
        data=req_json,
        timeout=30,
        proxy_url_override=proxy_url,   # <-- pinned
    )
...
def _download_archive_zip(inn: str, session_id: str, archive_url: str) -> bytes:
    r = _sbis_get(
        archive_url,
    ...
    if not content.startswith(b"PK\x03\x04"):
        raise RuntimeError("Ответ по СсылкаНаАрхив не ZIP (нет сигнатуры PK\\x03\\x04)")
    return content
...
def sbis_auth_session_for_inn(inn: str) -> dict:
    """
...
def sbis_rpc(inn: str, session_id: str, method: str, params: dict, *, timeout: int = 45) -> dict:
    """
    ...
    resp = _sbis_request("POST", REPORTING_URL, inn=inn, headers=headers, data=body_json, timeout=timeout)
