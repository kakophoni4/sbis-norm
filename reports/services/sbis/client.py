            ...
            if _is_retryable_http_status(resp.status_code) and (inn or proxy_url_override):
                ...
                if any(
                    ...
                    for x in (
                        ...
                        # биллинг СБИС — ретраи прокси бесполезны
                        "нет лицензии",
                        "отправляете отчеты в долг",
                        "отчеты в долг",
                        "отсутствует или обработано",
                        # пакет уже записан — нужен finish (Подготовить/Выполнить), не смена прокси
                        "уже запущен в обработку",
                        "передать отчет на подпись",
                        # PDF архива готовится на стороне СБИС — ждать, не крутить прокси
                        "pdf-файлы ещё формируются",
                        "еще формируются",
                        "ещё формируются",
                        "преобразовать файлы в pdf",
                        # доступ к документу — смена прокси не поможет
                        "недостаточно прав",
                        "недостаточно прав на просмотр",
                    )
                ):
                    logger.warning(
