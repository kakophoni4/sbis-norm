...
CONTAINER_NAME = "77fd6caf-4298-447a-872c-994f9a63a5c2 копия"
...
def get_certificate_from_container(container):
    cmd = f"{CERTMGR} -export -dest der -cont '{container}' -out temp_cert.der"
    run_command(cmd)

    if not os.path.exists("temp_cert.der"):
        raise ValueError("Certificate export failed")
...
def decrypt_session_key(encrypted_session_id, container):
    try:
        ...
        cmd = f"{CRYPTCP} -decr -f -cont '{container}' -in 'temp.enc' -out 'temp.dec'"
        run_command(cmd)
...
if __name__ == "__main__":
    try:
        ...
        if not encrypted_key:
            print("Error: No session key received")
            exit(1)
