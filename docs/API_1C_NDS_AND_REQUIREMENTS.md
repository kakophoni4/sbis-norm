...
| `counterparty_id` | для `format=pdf` — да | ИНН покупателя; пусто в json = все строки раздела 9 |
| `date_from` / `date_to` | нет | период |
| `format` | нет | `json` (по умолчанию) или `pdf` |

**`format=json`** — строки раздела 9 из XML архива.

**`format=pdf`** — PDF в стиле книги продаж Saby (раздел 9), только строки выбранного контрагента + тот же визуальный синий штамп исходного `ОтчетФНС`. В ответе: `pdf_b64`, `pdf_filename`, `matched_rows`, `stamp`.

Для **полной подписанной книги** (все контрагенты) — следующий метод.
